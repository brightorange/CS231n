...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    out = np.reshape(x, (x.shape[0], w.shape[0])).dot(w) + b
    ###############################)############################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_sh_sqrt = x_sh**2
        #step3
        sample_var = np.mean(x_sh_sqrt, axis=0)
        #step4
        sample_var_for_calc = np.sqrt(sample_var + eps)
        #step5
        denominator = 1/sample_var_for_calc
        #step6
        x_hat = x_sh * denominator
        #step7
        out = gamma * x_hat + beta

        cache = (x_hat, gamma, x_sh, x_sh_sqrt, denominator, sample_var_for_calc, sample_var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, D = dout.shape
    x_hat, gamma, x_sh, x_sh_sqrt, denominator, var_calc, var, eps = cache
    ...
    dx_hat = gamma * dout
    #step6
    ddenominator = np.sum(dx_hat * x_sh, axis=0)
    dx_sh1 = dx_hat * denominator
    #step5
    dvar_calc = -1. / (var_calc **2 ) * ddenominator
    #step6
    dvar = 1. / (2*np.sqrt(var + eps)) * dvar_calc
    #step5
    dsh_sq = 1. / N * np.ones((N,D)) * dvar
    #step4
    dx_sh2 = 2 * x_sh * dsh_sq
    #step3
    dx1 = dx_sh2 + dx_sh1
    dx2_1 =  -1 *np.sum(dx1, axis=0)
    #step 2
    dx2 = 1. / N * np.ones((N,D)) * dx2_1
    #step1
    dx = dx1 + dx2
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H2 = int(1 + (H + 2 * pad - H1) / stride)
    W2 = int(1 + (W + 2 * pad - W1) / stride)
    ...
    for n in range(N): # for all image
  	    for f in range(F): # for all filter
  	        for hh in range(H2): # height of activation map
	            for ww in range(W2): # width of activation map
...
def max_pool_forward_naive(x, pool_param):
    ...
    argOut = np.zeros((N, C, H1, W1))

    for n in range(N):
  	    for c in range(C):
  	        for h in range(H2):
  	  	        for w in range(W2):
  	  	            out[n, c, h, w] = np.max(x[n, c, h*S:h*S+FH,  w*S:w*S+FW])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
  	    for c in range(C):
  	        for h in range(H2):
  	  	        for w in range(W2):
  	  	  # better than using argmax and max together
  	  	            window = x[n, c, h*S:h*S+FH,  w*S:w*S+FW]
  	  	            zeros = window == np.max(window)
  	  	            dx[n, c, h*S:h*S+FH,  w*S:w*S+FW] = dout[n, c, h,  w] * zeros
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
